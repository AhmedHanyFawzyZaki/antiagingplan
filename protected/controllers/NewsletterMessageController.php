...
class NewsletterMessageController extends AdminController
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['NewsletterMessage']))
		{
			...
			{
				$mail = new YiiMailer();
				$mail->setFrom(Settings::model()->findByPk(1)->email,Yii::app()->name.' Admininstrator');
				//$mail->setTo($model->email);
				$mail->setSubject($model->subject);
				$mail->setBody($model->message);
				$users_id=explode(',',$model->users_id);
				foreach($users_id as $user)
				{
					$email=Subscriber::model()->findByPk($user)->email;
					$mail->setTo($email);
					if($mail->send())
					{
						//echo 'aa '.$email.'ad '.Yii::app()->params['adminEmail'];
					}
					else
					{
						//echo 'ff'.$email;
